...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 -s 192.168.1.10:8888,192.168.1.11:8888,192.168.1.12:8888 -t 10"
    echo "  $0 --servers 10.0.0.1:8888,10.0.0.2:8888,10.0.0.3:8888 --times 5"
    echo ""
    exit 1
}
...
if [ -z "$SERVERS" ]; then
    ...
    echo -e "${YELLOW}Please specify server list like: -s 192.168.1.10:8888,192.168.1.11:8888${NC}"
    usage
fi
