...
func (kvc *KVClient) SendGetInWritelessStrong(address string, request *kvrpc.GetInWritelessStrongRequest) (*kvrpc.GetInWritelessStrongResponse, error) {
	conn, err := grpc.Dial(address, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		util.EPrintf("err in SendGetInWritelessStrong: %v", err)
		return nil, err
	}
	defer conn.Close()
	client := kvrpc.NewKVClient(conn)
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
	reply, err := client.GetInWritelessStrong(ctx, request)
	if err != nil {
		util.EPrintf("err in SendGetIWritelessStrong: %v", err)
		return nil, err
	}
	return reply, nil
}

func (kvc *KVClient) SendPutInWritelesStrong(address string, request *kvrpc.PutInWritelessStrongRequest) (*kvrpc.PutInWritelessStrongResponse, error) {
	conn, err := grpc.Dial(address, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		util.EPrintf("err in SendPutInWritelessStrong: %v", err)
		return nil, err
	}
	defer conn.Close()
	client := kvrpc.NewKVClient(conn)
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
	reply, err := client.PutInWritelessStrong(ctx, request)
	if err != nil {
		util.EPrintf("err in SendPutInWritelessCausal: %v", err)
		return nil, err
	}
...
func (kvc *KVClient) PutInWritelessStrong(key string, value string) bool {
	request := &kvrpc.PutInWritelessStrongRequest{
		Key:         key,
		Value:       value,
		Vectorclock: kvc.Vectorclock,
		Timestamp:   time.Now().UnixMilli(),
	}
	// keep sending PutInWritelesStrong until success
	for {
		reply, err := kvc.SendPutInWritelesStrong(kvc.Kvservers[kvc.KvsId], request)
		if err != nil {
			util.EPrintf("err in PutInWritelesStrong: %v", err)
			return false
		}
		if reply.Vectorclock != nil && reply.Success {
			kvc.Vectorclock = reply.Vectorclock
		...
		// refresh the target node
		// util.DPrintf("PutInCausal Failed, refresh the target node")
		fmt.Printf("PutInWritelesStrong Failed, refresh the target node")
		kvc.KvsId = (kvc.KvsId + 1) % len(kvc.Kvservers)
	}
}

func (kvc *KVClient) GetInWritelessStrong(key string, value string) (string, bool) {
	request := &kvrpc.GetInWritelessStrongRequest{
		Key:         key,
		Vectorclock: kvc.Vectorclock,
	}
	for {
		request.Timestamp = time.Now().UnixMilli()
		reply, err := kvc.SendGetInWritelessStrong(kvc.Kvservers[kvc.KvsId], request)
		if err != nil {
			util.EPrintf("err in GetInWritelessStrong: %v", err)
			return "", false
		}
		if reply.Vectorclock != nil && reply.Success {
			kvc.Vectorclock = reply.Vectorclock
		...
		// refresh the target node
		// util.DPrintf("GetInCausal Failed, refresh the target node: %v", kvc.Kvservers[kvc.KvsId])
		fmt.Println("GetInWritelessStrong Failed, refresh the target node: ", kvc.Kvservers[kvc.KvsId])
		kvc.KvsId = (kvc.KvsId + 1) % len(kvc.Kvservers)
		atomic.AddInt32(&falseTime, 1)
	}
}
